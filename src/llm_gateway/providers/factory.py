...
def _config(name: str, base_url: str, supported_models: list[str]) -> ProviderConfig:
    ...
    return ProviderConfig(name=name, base_url=base_url, supported_models=supported_models)
...
def build_registry(settings: Settings | None = None) -> ProviderRegistry:
    ...
    if settings.OPENAI_API_KEY:
        providers.append(
            OpenAIProvider(
                _config("openai", OPENAI_BASE_URL, OPENAI_MODELS),
                client,
    ...
    if settings.GROQ_API_KEY:
        providers.append(
            OpenAIProvider(
                _config("groq", GROQ_BASE_URL, GROQ_SUPPORTED_MODELS),
                client,
    ...
    if settings.ANTHROPIC_API_KEY:
        providers.append(
            AnthropicProvider(
                _config("anthropic", ANTHROPIC_BASE_URL, ANTHROPIC_MODELS),
                client,
    ...
    if settings.MISTRAL_API_KEY:
        providers.append(
            MistralProvider(
                _config("mistral", MISTRAL_BASE_URL, MISTRAL_MODELS),
                client,
